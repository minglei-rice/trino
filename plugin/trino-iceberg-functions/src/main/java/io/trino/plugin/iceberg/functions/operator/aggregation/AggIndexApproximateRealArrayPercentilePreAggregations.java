...
public final class AggIndexApproximateRealArrayPercentilePreAggregations
{
    private AggIndexApproximateRealArrayPercentilePreAggregations() {}

    @InputFunction
    public static void input(@AggregationState AggIndexPercentileArrayAccumulatorState state, @SqlType(StandardTypes.REAL) long value, @SqlType("array(double)") Block percentilesArrayBlock)
    {
        inputArrayValue(state, intBitsToFloat((int) value), 1.0, percentilesArrayBlock);
    }

    @InputFunction
    public static void weightedInput(@AggregationState AggIndexPercentileArrayAccumulatorState state, @SqlType(StandardTypes.REAL) long value, @SqlType(StandardTypes.DOUBLE) double weight, @SqlType("array(double)") Block percentilesArrayBlock)
    {
        inputArrayValue(state, intBitsToFloat((int) value), weight, percentilesArrayBlock);
    }
