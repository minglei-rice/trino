...
@AggregationFunction("cube_avg_double")
public final class AggIndexDoubleAverageAggregations
{
    private AggIndexDoubleAverageAggregations() {}
    ...
    public static void output(@TypeParameter("T") Type type, @AggregationState AggIndexAccumulatorState state, BlockBuilder out)
    {
        if (state.resultIsNull()) {
            ...
            DoubleType.DOUBLE.writeDouble(out, (Double) state.getResult(type, AggIndex.AggFunctionType.AVG));
        }
    }
}
