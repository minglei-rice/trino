...
public final class BitmapIntAgg
{
    private BitmapIntAgg() {}

    private static final BitmapStateSerializer SERIALIZER = new BitmapStateSerializer();

    @InputFunction
    public static void input(@AggregationState BitmapState state, @SqlType(StandardTypes.INTEGER) long value)
    {
        if (state.getBitmap() == null) {
            state.setBitmap(new BitmapWithSmallSet(ClickHouseDataTypeMapping.INT32));
        }
        state.getBitmap().add((int) value);
    }
