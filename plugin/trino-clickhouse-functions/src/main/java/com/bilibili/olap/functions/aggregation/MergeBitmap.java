...
public final class MergeBitmap
{
    private MergeBitmap() {}
    ...
    @InputFunction
    public static void input(@AggregationState BitmapState state, @SqlType(BitmapType.NAME) Slice value)
    {
        BitmapWithSmallSet other = SERIALIZER.deserialize(value);
        merge(state, other);
    }
    ...
    {
        merge(state, otherState.getBitmap());
    }

    private static void merge(BitmapState state, BitmapWithSmallSet bitmap)
    {
        BitmapWithSmallSet previous = state.getBitmap();
        if (previous == null) {
            state.setBitmap(bitmap);
            state.addMemoryUsage(bitmap.estimatedMemorySize());
        }
        else {
            state.addMemoryUsage(-previous.estimatedMemorySize());
            previous.merge(bitmap);
            state.addMemoryUsage(previous.estimatedMemorySize());
        }
    }
