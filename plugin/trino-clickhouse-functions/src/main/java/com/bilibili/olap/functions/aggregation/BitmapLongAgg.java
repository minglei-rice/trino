...
/**
 * TODO: To define this function as a state function.
 */
@AggregationFunction("groupBitmap")
public final class BitmapLongAgg
{
    private BitmapLongAgg() {}

    private static final BitmapStateSerializer SERIALIZER = new BitmapStateSerializer();

    @InputFunction
    public static void input(@AggregationState BitmapState state, @SqlType(StandardTypes.BIGINT) long value)
    {
        if (state.getBitmap() == null) {
            state.setBitmap(new BitmapWithSmallSet(ClickHouseDataTypeMapping.INT64));
        }
        state.getBitmap().add(value);
    }
