 ...
 */
public class CustomizedGenericUDFBridge
        extends GenericUDFBridge
{
    ...
    {
        if (arguments.length != realArguments.length) {
            throw new RuntimeException("should the same");
        }
        ...
        if (isOperator) {
            if (children.length == 1) {
                ...
                if (children.length != 2) {
                    throw new RuntimeException("should equal to 2");
                }
    ...
    public static class CustomizedUDFMethodResolver
